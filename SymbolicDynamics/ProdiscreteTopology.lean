...
def eqOn_finite_entourages_generate_eq {A T: Type*}:
  Filter.generate (@eqOn_finite_entourages A T) = (@eqOn_finite_entourages_uniformSpace A T).uniformity :=
  sorry
...
theorem prodiscrete_uniform_structure_eq {A T: Type*} [UniformSpace A] [DiscreteUniformity A]:
  prodiscrete_uniform_structure A T = eqOn_finite_entourages_uniformSpace A T := by
  -- show their uniformities are equal
